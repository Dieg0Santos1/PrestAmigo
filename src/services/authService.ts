...
class AuthService {
  ...
  async register(data: RegisterData) {
    try {
      ...
      // 1. NORMALIZAR EL TELÉFONO ESTRICTAMENTE
      const normalizedPhone = normalizePhoneNumber(data.telefono);
      
      if (!normalizedPhone) {
        throw new Error('Número de teléfono inválido. Debe ser un número peruano válido.');
      }
      
      console.log('📱 Teléfono normalizado:', normalizedPhone);
      
      // 2. Verificar si el teléfono ya está registrado usando filtro en DB
      ...
      if (checkError) {
        console.error('Error verificando teléfono:', checkError);
      }
